...
#include "node/modbus/modbus_server_node_agent.h"

////////////////////////////////////////////////////////////////////////
//                         SERVER NODES                               //
////////////////////////////////////////////////////////////////////////

modbus_server_node_agent_t::modbus_server_node_agent_t
  ...
  , node_name_(node_name), node_address_(node_address), node_api_key_(node_api_key)
{
  ...
  mb_server_node_  = nullptr;

  if(node_type_.compare("modbus-server") == 0) {
    mb_server_node_ = std::make_shared<modbus_server_node_t>
      (controller_id_
...
modbus_server_node_agent_t& modbus_server_node_agent_t::process_events_fn
(std::function<void(std::shared_ptr<std::vector<modbus_node_event_t>>)> node_events_callback)
{
  ...
  if(node_type_.compare("modbus-server") == 0) {
    if(mb_server_node_) {
      modbus_server_node_t *sn = (modbus_server_node_t*)mb_server_node_.get();
...
modbus_server_node_agent_t& modbus_server_node_agent_t::init_agent()
{
  // Now initializa the node
  // Controller -> Agent -> Node {Modbus master node}
  if(mb_server_node_) {
    mb_server_node_->init_modbus_node();
  }
