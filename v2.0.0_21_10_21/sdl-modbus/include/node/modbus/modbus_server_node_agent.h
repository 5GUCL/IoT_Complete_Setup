...
#ifndef _CS_EDM_MODBUS_SERVER_NODE_AGENT_H_
#define _CS_EDM_MODBUS_SERVER_NODE_AGENT_H_
...
struct modbus_server_node_agent_t {
  modbus_server_node_agent_t
  ...
  modbus_server_node_agent_t& init_agent();
  modbus_server_node_agent_t& process_events_fn(std::function<void(std::shared_ptr<std::vector<modbus_node_event_t>>)> node_events_callback) ;
  ...
  std::shared_ptr<modbus_server_node_t> mb_server_node_;

  std::shared_ptr<std::thread> wrapper_mthread_;
};

#endif /* _CS_EDM_MODBUS_SERVER_NODE_AGENT_H_ */
