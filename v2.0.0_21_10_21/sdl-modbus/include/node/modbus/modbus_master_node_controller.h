...
#ifndef _CS_EDM_MODBUS_MASTER_NODE_CONTROLLER_H_
#define _CS_EDM_MODBUS_MASTER_NODE_CONTROLLER_H_
...
struct modbus_master_controller_per_socket_data_t {
  std::string_view user_secure_token;
};


/* uWebSocket worker runs in a separate thread */
struct modbus_master_controller_api_worker_t {
  void work();
  ...
  std::shared_ptr<std::vector<modbus_master_controller_api_worker_t>> all_api_workers_;
  std::string api_server_listen_address_;
  int api_server_listen_port_;

  so_5::mchain_t http_post_channel_;
  so_5::mchain_t ws_publish_channel_;
  std::string controller_logger_name_;
};

// Holds post data from field devices.
struct modbus_master_controller_http_post_mailbox_message_t {
  std::string msg_posted_;
};

// Publish the data to the WS clients...
struct modbus_master_controller_ws_mailbox_message_t {
  std::string msg_to_be_published_;
};

// Controller mailbox message
struct modbus_master_controller_mailbox_message_t { };
...
struct modbus_master_node_controller_t {
  ...
  modbus_master_node_controller_t
  ...
  std::string controller_logger_name_;
  // Node Agents
  std::vector<modbus_master_node_agent_t> mb_master_node_agents_vec_;
  
  // NCAP MQTT Backends
  std::vector<ncap_mqtt_backend_config_t> mb_ncap_mqtt_backend_vec_;
  ...
  std::shared_ptr<so_5::wrapped_env_t> mb_master_node_env_obj_;
  ...
  std::vector<modbus_master_controller_api_worker_t> api_workers_;
  std::string api_server_listen_address_;
  int api_server_listen_port_;
...
#endif /* _CS_EDM_MODBUS_MASTER_NODE_CONTROLLER_H_ */
