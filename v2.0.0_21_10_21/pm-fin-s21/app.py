...
# In Production, allowing all origins is NOT a good option.
...
def s21_mqtt_client_worker_fn():
    while True:
        ...
        logger_.info('Starting MQTT Client (s21_mqtt_client_worker_fn).')
        s21_mqtt_client.run()  # The client look shall not return.
        logger_.error('MQTT Client (s21_mqtt_client_worker_fn) restarted.')
...
@app_.route("/logout")
def logout():
    if s21_auth_blueprint_.session.authorized and s21_auth_blueprint_.token:
        access_token = s21_auth_blueprint_.token["access_token"]
        refresh_token = s21_auth_blueprint_.token['refresh_token']
        ...
        r = requests.post(s21_cfg_['auth']['end_session_endpoint'], data=payload_, headers=headers_)
        if r.status_code == 200 or r.status_code == 204:
            logger_.info('Keycloak session deleted!!')
        else:
            logger_.info('Keycloak session NOT deleted!!')
        ...
        flask_login.logout_user()  # Delete Flask-Login's session cookie
        s21_auth_blueprint_.token = None  # Delete OAuth token from storage
...
socketio.on_namespace(
    S21SupervisorNamespace('/supervisor', s21_tx_req_queue_, s21_auth_blueprint_, session, logger_, True))

socketio.on_namespace(
    S21SiteNamespace('/site', s21_tx_req_queue_, s21_auth_blueprint_, session, logger_, True))

socketio.on_namespace(
    S21GlobalNamespace('/global', s21_tx_req_queue_, s21_auth_blueprint_, session, logger_, True))

for ns in AllowedSites:
    socketio.on_namespace(
        S21SiteNamespace('/' + ns['ncap_id'], s21_tx_req_queue_, s21_auth_blueprint_, session, logger_, True))
