...
class S21GlobalNamespace(Namespace):
    ...
    # The SIO acknowledges with a session ID and original message back to the client.
    # The SIO adds the S21_TX_REQ_API Message to the Queue ->  s21_transmitter_worker_fn processes the message.
    ...
    def on_s21_global_ping(self):
        emit('s21_global_pong', namespace=self.name_space_)

    def on_ping(self):
        emit('pong', namespace=self.name_space_)

    # Client calls ACK callback upon receiving a message from the server.
    def s21_global_hello_api_ack(self):
        self.logger_.info('Client received Hello ACK message --> --> S21_GLOBAL_HELLO_ACK')

    # Client may send Hello Message once a successful connection is established.
    def on_s21_global_hello_api(self, message):
        self.logger_.info('Sending Hello Acknowledgement Message to the client ...')
        self.session_['s21_tx_req_api_session_id'] = self.session_.get('s21_tx_req_api_session_id', 0) + 1
        emit('s21_global_hello_api_ack',
             {
                 's21_global_hello_api': message['s21_global_hello_api'],
                 's21_global_hello_api_ack': 'Client Global Hello ACK Message',
                 's21_tx_req_api_session_id': self.session_['s21_tx_req_api_session_id']
             },
             namespace=self.name_space_,
             callback=self.s21_global_hello_api_ack)
    ...
    def on_connect(self):
        ...
        emit('s21_global_connect_response',
             {
                 'data': 'Connected',
                 'count': 0
             },
