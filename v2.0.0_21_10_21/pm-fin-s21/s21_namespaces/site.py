...
class S21SiteNamespace(Namespace):
    ...
    def on_s21_site_ping(self):
        emit('s21_site_pong', namespace=self.name_space_)

    def on_ping(self):
        emit('pong', namespace=self.name_space_)

    # Client calls ACK callback upon receiving a message from the server.
    def s21_site_hello_api_ack(self):
        self.logger_.info('Client received Hello ACK message --> --> S21_SITE_HELLO_ACK')

    # Client may send Hello Message once a successful connection is established.
    def on_s21_site_hello_api(self, message):
        self.logger_.info('Sending Hello Acknowledgement Message to the client ...')
        self.session_['s21_tx_req_api_session_id'] = self.session_.get('s21_tx_req_api_session_id', 0) + 1
        emit('s21_site_hello_api_ack',
             {
                 's21_site_hello_api': message['s21_site_hello_api'],
                 's21_site_hello_api_ack': 'Client Site Hello ACK Message',
                 's21_tx_req_api_session_id': self.session_['s21_tx_req_api_session_id']
             },
             namespace=self.name_space_,
             callback=self.s21_site_hello_api_ack)
    ...
    def on_connect(self):
        ...
        emit('s21_site_connect_response',
             {
                 'data': 'Connected',
                 'count': 0
             },
